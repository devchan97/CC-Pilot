...
## 위험 권한 설정 (모든 권한 확인 건너뛰기)

CCPilot은 기본적으로 모든 Claude Code 세션을 `--dangerously-skip-permissions` 플래그와 함께 실행합니다. 이는 **Claude가 파일 편집, 셸 명령 실행 등 모든 도구를 확인 없이 즉시 실행**한다는 의미입니다.

이것은 자동화 에이전트 워크플로를 위한 의도적 설계입니다 — 태스크를 지시하면 Claude가 자율적으로 실행합니다.
